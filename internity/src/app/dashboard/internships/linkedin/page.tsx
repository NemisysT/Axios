...
export default function LinkedInInternshipsPage() {
  ...
  const startLinkedInScraper = async () => {
    setIsLoading(true)
    try {
      const response = await fetch(" http://127.0.0.1:5000/api/linkedin/scrape", {
        method: "POST",
        headers: {
  ...
  const fetchLinkedInInternships = async () => {
    try {
      const res = await fetch("http://127.0.0.1:5000/api/linkedin/list", {
        method: "GET",
      })
  ...
  const handleScrape = async () => {
    await startLinkedInScraper()
    await fetchLinkedInInternships()
  }
